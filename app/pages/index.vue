...
const columns: TableColumn<ProviderPlan>[] = [
        ...
        cell: ({ row }) =>
            h(
                ProviderPopover,
                {
                    ...
                    providerUrl: row.original.providerUrl,
                    protocols: row.original.protocols,
                },
                h(UButton, row.original.providerName),
            ),
    },
    {
        accessorKey: 'name',
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            return storage !== undefined
                ? (storage / 1e9).toLocaleString(undefined, {
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            return traffic === -1
                ? 'Unlimited'
                : (traffic / 1e9).toLocaleString(undefined, {
                      maximumFractionDigits: 2,
                  }) + ' GB';
        },
    },
...
<template>
    <UDashboardPanel id="home" :ui="{ body: 'gap-0 sm:gap-0 p-0 sm:p-0' }">
        <template #header>
            ...
            <UDashboardToolbar>
                <template #default>
                    <UForm ref="formRef" :schema="schema" :state="state" class="flex w-full flex-col items-center gap-2 py-1">
                        <div class="flex w-full flex-1 flex-row flex-wrap items-center gap-4">
                        ...
                        <div class="flex w-full flex-row flex-wrap items-center gap-4">
                            <UFormField label="Protocol(s)" name="protocols" class="flex-1">
                                <USelectMenu
                                    v-model="state.protocols"
                                    multiple
                                    label-key="label"
                                    value-key="value"
                                    :items="
                                        storageProtocols.map((protocol) => ({
                                            label: protocol as string,
                                            value: protocol,
                                        }))
                                    "
                                    class="w-full"
